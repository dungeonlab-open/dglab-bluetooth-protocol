...
70 指令用于设置脉冲主机的电量指示灯颜色，指令数据长度为 2 字节，写入 0x150A 特性后直接生效，没有返回值。

```
0x70(1byte指令HEAD)+颜色值(1byte)
```

颜色值范围为 `0x00` 到 `0x08`。目前该值只确认是设备接受的原始颜色选项，暂未整理出稳定的颜色名称映射；建议开发时先逐个写入测试，再在自己的应用中显示对应颜色名称。

> e.g<br />设置电量指示灯为颜色选项 3：`0x7003`
